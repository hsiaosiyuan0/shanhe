...
<p align="center"><sub>真实界面 · 苏轼的一生 · 地形、事件与对话共处一个工作台</sub></p>
...
三个示例开箱可用，也可以创建空白故事，从自己的主题开始。自由主题的内容补充需要连接真实模型。
...
- **河道上的名字**：长江、黄河沿河道着色与标注，放大后继续阅读河段。
- **现代行政区对照**：一键叠加现代省界、省名，点击地图查看所属省份。
...
探索助手可以读取当前故事，并实际操作地图：**添加事件、标记地点、绘制路线、调整图层与视角**。文字回答和地图变更一起保存。
...
| 自动保存                                                                                                      | 版本快照                                             | 完整迁移                                                    |
| :------------------------------------------------------------------------------------------------------------ | :--------------------------------------------------- | :---------------------------------------------------------- |
| 编辑和完成的对话写入本地存储（桌面版 SQLite，在线版 IndexedDB）。对话中途失败或取消，暂存的地图修改不会落盘。 | 保存故事、对话与地图视角；恢复之前自动备份当前版本。 | 导出事件、路线、标记和聊天为 JSON，导入另一台设备继续探索。 |
...
**直接打开 [在线版](https://hsiaosiyuan0.github.io/shanhe/)**，无需安装。支持故事编辑、地图、快照和浏览器保存；连接模型需要支持 CORS 的 HTTPS API，密钥仅在当前页面内存中使用。本机 Codex、Ollama 请使用本地版。[版本差异与部署说明](docs/PAGES.md)
...
当前是未签名、未公证的本地开发构建。Windows / Linux 可以使用浏览器版本，暂无对应桌面包。[其他运行与打包方式](docs/SETUP.md#运行)
...
| 文档                                     | 内容                                                     |
| :--------------------------------------- | :------------------------------------------------------- |
| [在线版与部署](docs/PAGES.md)            | 浏览器保存、模型跨域连接、版本差异与 GitHub Actions      |
...
<p align="center">
  <sub>地图数据：Natural Earth · Esri / USGS · Mapzen / AWS<br />
