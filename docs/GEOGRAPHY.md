...
- 现代市县：通过 [OpenFreeMap](https://openfreemap.org/quick_start/) 按视野加载 OpenStreetMap 矢量瓦片。「今地对照」开启后，缩放 5 级起显示城市地名，7 级起显示市界，8 级起显示区县边界与地名，11 级起补充乡镇和街区。省、市、区县界使用不同粗细与线型，浅色衬边帮助它们从地形中显现；左上角图例提示当前层级。地名会避让，省名在放大时淡出。覆盖与更新取决于源数据，未离线打包所有市县；加载失败时保留内置省级参照，并可重试。
