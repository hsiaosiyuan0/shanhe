...
- 湖泊与水库：使用 HydroLAKES v1.0 的 44,492 个完整水面要素，中国区域约 1:25 万，保留源坐标和内洞并按视野加载。洞庭湖命名面只覆盖局部；「水面核对」可切换到 JRC 1984–2024 长期观测图。二者均非实时或古代湖岸。来源、覆盖例外、数据获取进度见[湖泊数据说明](LAKE_DATA.md)。
