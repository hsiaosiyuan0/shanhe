...
- 现代省、市、区县：统一使用[天地图行政区划](https://cloudcenter.tianditu.gov.cn/administrativeDivision/)的 2025 年 9 月版下载包，包含 34 / 375 / 2,891 个面要素及独立境界线。CGCS2000 经纬度原值按区域尺度近似显示，没有手工补画；仅供地图可视化使用。缩放 7 级以下显示省级、7–8 级显示市级、8 级起显示区县，一次仅绘制一个层级的公共边界。悬停使用同包多边形，不另请求轮廓。原始文件、哈希、精度限制和几处源数据交叠见[行政区数据说明](ADMIN_DATA.md)。
